...
def nanquantile_along_axis(
    data: np.ndarray, q1: float, q2: float, transpose: bool = True
) -> np.ndarray:
    """Numba quantile."""
...
def _precompile():
    ...
    if (
        os.environ.get("KOYO_JIT_PRE", "0") == "0"
        or os.environ.get("IMIMSPY_JIT_PRE", "0") == "0"
    ):
        return
